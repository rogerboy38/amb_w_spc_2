...
class CustomStockEntry(StockEntry):
	...
	def on_submit(self):
		...
		self.trigger_system_integrations()
	...
	def trigger_system_integrations(self):
		"""Trigger integration with all related systems"""
		try:
			...
			# Enhanced batch integration for all items with batches
			for item in self.items:
				if item.batch_no:
					WarehouseIntegration.integrate_with_batch_amb(item.batch_no, self.name)
			
			# Comprehensive batch-warehouse integration
			WarehouseBatchIntegration.create_stock_entry_batch_history(self)
			WarehouseBatchIntegration.update_warehouse_bin_batch_tracking(self)
			WarehouseBatchIntegration.validate_batch_warehouse_movement(self)
			
			# Work Order integration
			if self.custom_work_order_reference:
				WarehouseIntegration.integrate_with_mrp_planning(self.custom_work_order_reference)
				WarehouseBatchIntegration.sync_warehouse_zone_status_with_batch_quality(self.custom_work_order_reference)
			...
			frappe.log_error(f"System integration failed for Stock Entry {self.name}: {str(e)}")
			# Don't fail the stock entry submission due to integration issues
...
def before_submit(doc, method=None):
	...
	# Enhanced batch-warehouse validation
	try:
		from amb_w_spc.sfc_manufacturing.warehouse_management.warehouse_batch_integration import WarehouseBatchIntegration
		WarehouseBatchIntegration.validate_batch_warehouse_movement(doc)
	except Exception as e:
		frappe.log_error(f"Batch-warehouse validation failed: {str(e)}")
